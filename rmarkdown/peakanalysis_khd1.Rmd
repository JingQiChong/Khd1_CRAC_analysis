...
---
#Summary
This analysis searches for motifs associated with Khd1-bound RNA from the CRAC datasets using the MEME software.
This script relies on Khd1-bound peak data in gtf format file produced by `pyCalculateFDRs.py` software in the nf_CRACpipeline and the genome sequence in fasta format (Saccharomyces_cerevisiae.EF4.74.dna.toplevel.shortChrNames.fa) as input.

This script filter Khd1 CRAC hits (peaks) by read count, width and false dicovery rate (FDR) and then generate fasta files that contains the sequences of the filtered peaks. Lastly, the MEME software is run to search for motifs enriched in the filtered peaks.
...
```{r load_gffs}
SRR847751_df <- read_FDR_gff("~/CRAC_analysis_2022/Khd1_CRAC_analysis/Khd1_SRR847751/pyCalculateFDRs_analyses/SRR847751_trimmed_output_FDRs.gtf")
...
```{r find_seqs_setup}
S288C_FaF <- Rsamtools::FaFile("~/Khd1_CRAC_analysis/input_annotation/Saccharomyces_cerevisiae.EF4.74.dna.toplevel.shortChrNames.fa")
